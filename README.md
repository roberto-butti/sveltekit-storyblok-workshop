...
Now, when you run `bun run dev`, the SvelteKit application is served via the default `5173` port and the HTTPS protocol instead of the usual HTTP protocol.
...
## Installing the Storyblok Svelte SDK
...
Make sure that the `vitePreprocess` function is imported from `vite`, in the `svelte.config.js` file:
...
Now you can run `bun run build` for generating static pages.
